...
import { realtimeService, RealtimeConnectionStatus, RealtimeEvent } from '@/lib/realtime/realtime-service'
import { StatisticsCalculator, ApplicationRecord } from '@/lib/statistics/statistics-calculator'
...
export function useRealtimeStatistics(
  ...
  const enableRealtime = options.enableRealtime ?? DEFAULT_OPTIONS.enableRealtime
  ...
  const handleRealtimeEvent = useCallback((event: RealtimeEvent<ApplicationRecord>) => {

    // Check for authorization errors
    if (event.errors?.some(error => error.includes('401') || error.includes('Unauthorized'))) {
      const authError = new Error('Unauthorized: Please check RLS policies for realtime access')
      setError(authError)
    ...
    setStatistics(currentStats => {
      ...
      try {
        ...
        if (!StatisticsCalculator.validateStatistics(updatedStats)) {
          console.warn('Statistics validation failed, falling back to API fetch')
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      ...
      const error = err instanceof Error ? err : new Error('Failed to refetch statistics')
      setError(error)
      onErrorRef.current(error)
    } finally {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        const error = err instanceof Error ? err : new Error('Failed to initialize statistics')
        setError(error)
        setConnectionStatus('error')
        onErrorRef.current(error)
      } finally {
