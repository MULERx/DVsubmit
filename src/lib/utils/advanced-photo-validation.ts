 ...
 */
export async function validateAdvancedPhotoCompliance(
  file: File
): Promise<AdvancedPhotoValidationResult> {
  const errors: string[] = [];
  ...
  try {
    ...
    if (
      qualityAssessment.brightness < 40 ||
      qualityAssessment.brightness > 80
    ) {
    ...
    return {
      isValid: false,
      errors: [
        "Failed to analyze photo. Please try again with a different image.",
      ],
      warnings: [],
      complianceScore: 0,
...
function analyzeImageQuality(data: { imageData: ImageData }): {
  ...
  const qualityScore =
    (sharpness + Math.min(brightness, 100 - brightness) + contrast) / 3;
...
function analyzeBackground(data: { imageData: ImageData }): {
  ...
  for (let x = 0; x < width; x++) {
    ...
    const topIndex = x * 4;
    const topGray =
      (pixels[topIndex] + pixels[topIndex + 1] + pixels[topIndex + 2]) / 3;
  ...
  for (let y = 0; y < height; y++) {
    ...
    const leftGray =
      (pixels[leftIndex] + pixels[leftIndex + 1] + pixels[leftIndex + 2]) / 3;
    backgroundPixels.push(leftGray);

    // Right edge
    const rightIndex = (y * width + width - 1) * 4;
    const rightGray =
      (pixels[rightIndex] + pixels[rightIndex + 1] + pixels[rightIndex + 2]) /
      3;
  ...
  const avgBackground =
    backgroundPixels.reduce((sum, val) => sum + val, 0) /
    backgroundPixels.length;
  const backgroundVariance =
    backgroundPixels.reduce(
      (sum, val) => sum + Math.pow(val - avgBackground, 2),
      0
    ) / backgroundPixels.length;
  ...
  const backgroundColor =
    avgBackground > 200 ? "light" : avgBackground > 100 ? "medium" : "dark";
