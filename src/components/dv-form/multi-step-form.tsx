...
export function MultiStepForm({
  ...
  const { formData, updateStepData, setFormData } =
    useFormData(existingApplication);
  const {
    currentStep,
    goToNextStep,
    goToPreviousStep,
  ...
  const isLoading =
    applicationSubmission.isPending || applicationUpdate.isPending;
  ...
  const validateFormData = (data: Partial<FormStepData>) => {
    ...
    if (
      ...
      return {
        isValid: false,
        error: `Missing required form data: ${missingSteps.join(
          ", "
    ...
    const requiredFields = [
      ...
      {
        field: data.personal.countryOfEligibility,
        name: "Country of Eligibility",
      },
  ...
  const handleFinalSubmit = useCallback(
    async (transactionNumber: string) => {
      try {
        ...
        const applicationData = transformFormDataForSubmission(
          updatedFormData as FormStepData
        );
        ...
        if (isEditing) {
          ...
          const result = await applicationSubmission.mutateAsync(
            applicationData
          );
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      ...
      case "children":
        // Debug logging
        console.log(
          "MultiStepForm - Rendering children step with photos:",
          formData.childrenPhotos
        );
        return (
          <ChildrenForm
    ...
    <div className="max-w-4xl mx-auto px-3">
      ...
      <div className="mb-3">
        <div className="text-sm text-gray-500">
          Complete all steps to submit your application. Your data is only saved
          when you submit the final form.
        </div>
      </div>
