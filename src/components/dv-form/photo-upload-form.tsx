...
export function PhotoUploadForm({
  ...
  const [preview, setPreview] = useState<string>(
    initialData?.signedUrl || initialData?.preview || ""
  );
  ...
  const [isExistingPhoto, setIsExistingPhoto] = useState<boolean>(
    !!initialData?.path
  );
  ...
  const validatePhoto = useCallback(
    async (file: File): Promise<PhotoValidationResult> => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-2">
          Upload a recent passport-style photo that meets DV lottery
          requirements.
        </p>
      </div>
                  ...
                  <div>
                    ...
                    {validation?.metadata && (
                      <p className="text-sm text-gray-500">
                        ...
                        {validation.metadata.height} pixels
                      </p>
                    )}
